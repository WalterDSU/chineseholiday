                ...
                if t == 2: #找到假日
                    ...
                    if (date - today).days >= min_days and (date - today).days <= max_days:
                        ...
                        for item in before_start_workdays:
                            date = item['date']
                            invert = item['invert']
                            before += " {}/{}(周{})".format(date.month,date.day,date.weekday()+1)
                            if invert:
                                before += "<补班>".format(date.weekday()+1) 
                        for item in after_end_workdays:
                            date = item['date']
                            invert = item['invert']
                            after += " {}/{}(周{})".format(date.month,date.day,date.weekday()+1)
                            if invert:
                                after += "<补班>".format(date.weekday()+1) 
                        info = "{}(周{})-{}(周{}) 放假共{}天\n节前需工作{}天 {} \n节后需工作{}天 {}".format(start.strftime('%m/%d'),start.weekday()+1,end.strftime('%m/%d'),end.weekday()+1,(end-start).days+1,(start-last_weekend).days-1,before,(next_weekend-end).days-1,after) #节日前后调休后需要连续工作多少天
